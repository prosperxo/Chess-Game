# Author: Andy Vo
# GitHub username: prosperxo
# Date: 03/24/24
# Description: Chess Var game with a twist! There is a falcon and hunter piece instead.

class Board:
    """Represents board with all the chess pieces """
    ...
    def __init__(self) -> None:
        ...
        self._board = [
            [self._pieces["black rook a"], self._pieces["black knight b"], self._pieces["black bishop c"],
             self._pieces["black queen"], self._pieces["black king"], self._pieces["black bishop f"],
             self._pieces["black knight g"], self._pieces["black rook h"]],
            [self._pieces["black pawn a"], self._pieces["black pawn b"], self._pieces["black pawn c"],
             self._pieces["black pawn d"], self._pieces["black pawn e"], self._pieces["black pawn f"],
             self._pieces["black pawn g"], self._pieces["black pawn h"], ],
            [self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], self._pieces["empty"],
             self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], ],
            [self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], self._pieces["empty"],
             self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], ],
            [self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], self._pieces["empty"],
            ...
            [self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], self._pieces["empty"],
             self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], self._pieces["empty"], ],
            [self._pieces["white pawn a"], self._pieces["white pawn b"], self._pieces["white pawn c"],
             self._pieces["white pawn d"], self._pieces["white pawn e"], self._pieces["white pawn f"],
             self._pieces["white pawn g"], self._pieces["white pawn h"], ],
            [self._pieces["white rook a"], self._pieces["white knight b"], self._pieces["white bishop c"],
             self._pieces["white queen"], self._pieces["white king"], self._pieces["white bishop f"],
             self._pieces["white knight g"], self._pieces["white rook h"]],
        ]
    ...
    def show_board(self):
        ...
        print(*letters, sep="\t")
        print("    ┌───────────────────────────────────┐")
        for row in range(8):  # Iterate through rows in ascending order
            print(8 - row, end="\t")  # Print reversed row number
            print("│", end="\t")
            for col in range(8):
            ...
            print("│", end="\t")
            print(8 - row)  # Print reversed row number
        print("    └───────────────────────────────────┘")
        print("  ", end="\t")
        print(*letters, sep="\t")
    ...
    def move_piece(self, origin_column: int, origin_row: int, destination_column: int, destination_row: int):
        ...
        piece_to_move = self._board[origin_row][origin_column]

        # Clear the origin square
        self._board[origin_row][origin_column] = self._pieces["empty"]

        # Place the piece to the destination square
        self._board[destination_row][destination_column] = piece_to_move
...
class Pawn(Piece):
    ...
    def get_valid_destinations(self, origin_column: int, origin_row: int, board):
        ...
        if board.is_on_board(trial_column_forward1, trial_row_forward1) and board.is_empty(trial_column_forward1,
                                                                                           trial_row_forward1):
            ...
            if self._moves_made == 0 and board.is_on_board(trial_column, trial_row) and board.is_empty(trial_column,
                                                                                                       trial_row):
        ...
        if board.is_on_board(trial_column_diagleft, trial_row_diagleft) and board.get_piece_at_coord(
                trial_column_diagleft, trial_row_diagleft).get_color() != self._color and not board.is_empty(
                trial_column_diagleft, trial_row_diagleft):
        ...
        if board.is_on_board(trial_column_diagright, trial_row_diagright) and board.get_piece_at_coord(
                trial_column_diagright, trial_row_diagright).get_color() != self._color and not board.is_empty(
                trial_column_diagright, trial_row_diagright):
...
class Knight(Piece):
    ...
    def get_valid_destinations(self, origin_column: int, origin_row: int, board):
        ...
        for (row_offset, column_offset) in possible_moves:
            ...
            if board.is_on_board(trial_column, trial_row) and board.get_piece_at_coord(trial_column,
                                                                                       trial_row).get_color() != self._color:
                valid_destinations.append((trial_column, trial_row))
...
class Bishop(Piece):
    ...
    def get_valid_destinations(self, origin_column: int, origin_row: int, board):
        ...
        for (row_offset, column_offset) in possible_moves:
            ...
            while board.is_on_board(trial_column, trial_row) and board.get_piece_at_coord(trial_column,
                                                                                          trial_row).get_color() != self._color:
                valid_destinations.append((trial_column, trial_row))
                if board.get_piece_at_coord(trial_column, trial_row).get_color() != self._color and not board.is_empty(
                        trial_column, trial_row):
                    break
                column_counter += column_offset
                row_counter += row_offset
                trial_column, trial_row = self.move(origin_column, origin_row, row_counter, column_counter)
...
class ChessVar:
    ...
    def _origin_or_destination_are_invalid(self, origin_column: int, origin_row: int, destination_column: int,
                                           destination_row: int) -> bool:
        """Returns True if either origin or destination coordinates aren't on the board, false otherwise"""
        return (not self._board.is_on_board(origin_column, origin_row)) or (
            not self._board.is_on_board(destination_column, destination_row))
    ...
    def make_move(self, origin: str, destination: str) -> bool:
        ...
        valid_destinations = self._board.get_piece_at_coord(origin_column, origin_row).get_valid_destinations(
            origin_column, origin_row, self._board)
    ...
    def enter_fairy_piece(self, fairy_piece_name: str, destination: str) -> bool:
        ...
        if destination_row not in home_rows or not self._board.is_on_board(destination_column,
                                                                           destination_row) or not self._board.is_empty(
        ...
        if self._board.count_major_pieces(self._current_turn) == 7:
            return False
        if self._board.count_major_pieces(self._current_turn) == 6 and self._board.count_fairy_pieces(
...
def main():
    ...
    # Make another move (black pawn d7 to d5)
    game.make_move("d7", "d5")
    #
    # Display the updated board state after the move

    # Enter a fairy piece (white falcon to c1)
    game.enter_fairy_piece("F", "c1") #should not happen because no piece is conquered

    # Display the updated board state after entering the fairy piece

    # Make another move (black pawn e7 to e5)
    game.make_move("e7", "e5")

    # Display the updated board state after the move

    # Make an invalid move (white pawn f2 to f4)
    # This move is invalid because it's not white's turn
    game.make_move("f2", "f4")  # This should return False

    # Enter an invalid fairy piece (white hunter to d4)
    # This move is invalid because the destination square is not in the home row
    game.enter_fairy_piece("H", "d4")  # This should return False
